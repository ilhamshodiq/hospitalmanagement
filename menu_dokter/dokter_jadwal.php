...
<body>
    ...
    // $sql = "SELECT * FROM jadwal_dokter WHERE id_dokter = '$id_dokter'";
    $sql = "SELECT * FROM jadwal_dokter, dokter WHERE jadwal_dokter.id_dokter = dokter.id_dokter AND jadwal_dokter.id_dokter = '$id_dokter'";
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <h2 class="fs-2 m-0">Jadwal Dokter, dr.<?php echo $nama ?></h2>
                </div>
            </nav>
            <div class="container-sm">
                <hr class="border-light border-2 opacity-75">
                <a class="btn btn-primary mb-2 btn-sm" href="dokter_tambah_jadwal.php" role="button">Tambah</a>
                    ...
                    <tbody>
                        <?php while ($data = $query->fetch_array()) { ?>
                            <tr>
                                ...
                                <td><?php echo $data['waktu_berakhir'] ?></td>
                                <td>
                                    <form action="dokter_edit_jadwal.php?id_jadwal_dokter= <?php echo $data['id_jadwal_dokter'] ?>" method="post">
                                        <input type="hidden" name="id_jadwal_dokter">
                                        <input class="btn btn-success btn-sm" type="submit" value="edit">
                                    </form>
                                    <p></p>
                                    <form action="aksi/aksi_hapusjadwal.php" method="post">
                                        <input type="hidden" name="id_jadwal_dokter" value="<?php echo $data['id_jadwal_dokter'] ?>">
                                        <input class="btn btn-danger btn-sm" type="submit" name="hapus" value="hapus" onClick="return confirm('Apakah Anda ingin menghapus jadwal?')">
                                    </form>
                                </td>
                            </tr>
                        <?php } ?>
